...
const history = () => {
	...
	const [isHumiVisible, setIsHumiVisible] = React.useState(false);
	const [isSoilVisible, setIsSoilVisible] = React.useState(false);
	...
	React.useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch(`${config.baseURL}/avg-data`);
				...
				if (data.status === 'success') {
					...
					const labels = data.data
						.slice(0, 7)
						.map((item) => new Date(item.ts).toLocaleDateString());

					const temperatures = data.data
						.slice(0, 7)
						.map((item) =>
							typeof item.temperature === 'number' && isFinite(item.temperature)
								? item.temperature
								: 0
						);

					const humidities = data.data
						.slice(0, 7)
						.map((item) =>
							typeof item.humidity === 'number' && isFinite(item.humidity)
								? item.humidity
								: 0
						);

					const soilMoistures = data.data
						.slice(0, 7)
						.map((item) =>
							typeof item.soilMoisture === 'number' &&
			...
			<SafeAreaView>
				<View style={styles.camera}>
					...
					<ScrollView style={styles.chartContainer}>
						...
						<TouchableOpacity onPress={() => setChartVisible(!isChartVisible)} style={styles.buttonStyle}>
							<Text>{isChartVisible ? 'Hide Temperature Chart' : 'Show Temperature Chart'}</Text>
						</TouchableOpacity>
						{isChartVisible && (
							<LineChart
						...
						<TouchableOpacity onPress={() => setIsHumiVisible(!isHumiVisible)} style={styles.buttonStyle}>
							<Text>{isHumiVisible ? 'Hide Humidity Chart' : 'Show Humidity Chart'}</Text>
						</TouchableOpacity>
                        ...
                        <TouchableOpacity onPress={() => setIsSoilVisible(!isSoilVisible)} style={styles.buttonStyle}>
							<Text>{isSoilVisible ? 'Hide Soil Moisture Chart' : 'Show Soil Moisture Chart'}</Text>
						</TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    buttonStyle:{
        marginTop: 5,
        backgroundColor: '#4fc978', // Change this to your preferred color
        padding: 10,
        borderRadius: 5,
        alignItems: 'center',
    }
